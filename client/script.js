...
// CODE HERE
let helloBtnMouseover = event => {
    event.target.classList.add('hello-mouseover')
}

sayHelloButton.addEventListener('mouseover', helloBtnMouseover)
...
// CODE HERE
let helloBtnMouseOut = event => {
    event.target.classList.remove('hello-mouseover')
}

sayHelloButton.addEventListener('mouseout', helloBtnMouseOut)
...
const repeatMyParam = () => {
    //YOUR CODE HERE
    axios.get('http://localhost:3000/repeat/BearsBeetsBattlestarGalactica')
        .then(res => {
...
let queryFunc = () => {
    axios.get('http://localhost:3000/query-test/?name=jeff/?lastname=derbes')
        .then(res => {
            console.log(res.data)
        })
}
...
// const addFoodBtn = document.querySelector('#foodbutton')

// addFoodBtn.addEventListener('submit', createFood)

const createFood = (event) => {
    event.preventDefault()
    const foodInput = document.querySelector('#foodbutton')
    const body = {
        newFood: foodInput.value
    }
    axios.post("http://localhost:3000/food", body)
...
const addFood = document.querySelector('#foodbutton')
